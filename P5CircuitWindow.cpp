...
P5CircuitWindow::P5CircuitWindow(QGraphicsScene *scene, QObject *parent)
    ...
    connect(pb1Button, &PB1Button::pressed, this, &P5CircuitWindow::handlePB1Pressed);
    connect(pb1Button, &PB1Button::released, this, &P5CircuitWindow::handlePB1Released);

    //connect(nfb, &CircuitComponent::pressed, this, &P5CircuitWindow::handlePB1Released);

    // Connect PB2 button signal for motor start
    connect(pb2, &CircuitComponent::pressed, this, &P5CircuitWindow::handlePB2Pressed);
    connect(cos1, &CircuitComponent::pressed, this, &P5CircuitWindow::handlecos1Pressed);

    // Connect TH_RY abnormal state
    connect(thRy, &CircuitComponent::pressed, this, &P5CircuitWindow::handleTHRYTriggered);

    resetCircuit(); // Initialize circuit
}

void P5CircuitWindow::handlecos1Pressed() { //cos1 true:1, cos1 false:2
    ...
    if (cos1->isActive() && thRy->isActive()) {
        bz->setOn(true);
        onBzLightUp();
        pl3->setOn(false);
    } else if(thRy->isActive()){
        bz->setOn(false);
        stopBzSound();
        pl3->setOn(true);
    }
...
void P5CircuitWindow::handleTHRYTriggered() {
...
void P5CircuitWindow::stopMotor() {
    mc1->setActive(false);
    mc2->setActive(false);
    mc3->setActive(false);
    t1->setActive(false);
...
void P5CircuitWindow::onBzLightUp() {
...
void P5CircuitWindow::stopBzSound() {
    if (player->playbackState() == QMediaPlayer::PlayingState) {
